/** Serve the selected local ZIP for Mochi (slot 1) or Iansui (slot 2). */
...
try{
 ...
 if(source===undefined){$notify("LINE 雙欄字型找不到 ZIP",`第 ${slot} 欄｜${item.label}`,item.file);$done({status:"HTTP/1.1 404 Not Found",body:"not found"});}
  ...
  console.log(`[LINE-Font-Dual] combo=${selected} slot=${slot} font=${item.label} size=${total}`);
  $notify("LINE 雙欄字型 ZIP 注入成功",`第 ${slot} 欄｜${item.label}`,`${total} bytes｜MD5 ${item.md5}`);
  $done({status:"HTTP/1.1 200 OK",headers,bodyBytes});
 }
}catch(e){console.log("[LINE-Font-Dual] ZIP error "+String(e));$done({status:"HTTP/1.1 500 Internal Server Error",body:String(e)});}
