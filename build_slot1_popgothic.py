#!/usr/bin/env python3
"""Build Pop Gothic CJK TC Bold for LINE's Mochi free slot (fontId 41)."""
import copy, hashlib, json, struct, zlib
from pathlib import Path
from fontTools.ttLib import TTFont
R=Path('/var/minis/workspace/line-custom-font')
src=Path('/var/minis/attachments/uploads/PopGothicCjkTc-Bold.ttf')
...
ttf=R/'Mochi-slot-popgothic.ttf';f.save(ttf,reorderTables=False)
...
out=R/'LINE-font-slot1-popgothic.zip';out.write_bytes(arc)
cm={}
for t in f['cmap'].tables:
 if t.isUnicode():cm.update(t.cmap)
required='傳送圖片這過還關開繁體中文';missing=[x for x in required if ord(x) not in cm]
