/**
 * LINE Romantic Round Local Echo v8
 * Reads iCloud Drive/Quantumult X/Data/LINE-font-romantic-v8-unsigned.zip.
 */
...
try {
  ...
  if (source === undefined) {
    console.log("[LINE-Font-v8] 已触发，但找不到：" + FILE);
    $notify("LINE 浪漫雅圆 v8", "找不到字型包", "请放入 iCloud Drive/Quantumult X/Data/" + FILE);
    $done({status:"HTTP/1.1 404 Not Found",headers:{"Content-Type":"text/plain"},body:"romantic v8 not found"});
  } else {
    ...
    console.log(`[LINE-Font-v8] 浪漫雅圆已读取：${status} ${start}-${end}/${total}`);
    $notify("LINE 浪漫雅圆 v8", "真正自定义包注入成功", `回传 ${end-start+1}/${total} bytes`);
    $done({status,headers,bodyBytes});
  }
} catch (e) {
  console.log("[LINE-Font-v8] 异常："+String(e));
